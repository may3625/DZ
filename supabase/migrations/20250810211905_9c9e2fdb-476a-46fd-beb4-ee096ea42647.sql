-- Phase 9 — Sécurité & Gouvernance: Audit triggers + Rétention/Archivage
...
DECLARE
  ...
  diff jsonb;
BEGIN
  -- Actor (auth.uid() disponible en contexte RLS)
  BEGIN
    actor := auth.uid();
  ...
  -- Identifier la clé (suppose une colonne id uuid)
...
-- Activer RLS sur audit_logs si non activé (sécuriser la consultation)
...
-- 2) Déclencheurs d'audit sur tables sensibles
-- Nettoyage puis création pour éviter doublons
...
-- 3) Archivage & Rétention — Loi 18-07 (baseline)
...
-- Paramétrage de rétention (admin-only)
...
-- Fonction d'archivage explicite
...
-- Purge selon rétention: supprime versions des textes archivés au-delà du délai
...
-- Nécessite l'extension pg_cron activée sur le projet
DO $$ BEGIN
  PERFORM 1 FROM cron.job WHERE jobname = 'purge-archived-legal-text-versions-daily';
  IF NOT FOUND THEN
