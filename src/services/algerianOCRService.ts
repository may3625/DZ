...
class AlgerianOCRService {
  ...
  async processDocument(
    ...
    try {
      ...
      // Corrections OCR avancées pour l'arabe (séparation, RTL, ligatures, etc.)
      try {
        const langForCorrections = detectedLanguage;
        if (langForCorrections === 'ara' || langForCorrections === 'mixed') {
          const { correctedText } = correctArabicOCR(finalText);
          finalText = correctedText;
        }
      } catch (e) {
        console.warn('⚠️ Échec des corrections OCR arabes:', e);
      }
