...
export function useAdvancedAI() {
  ...
  const extractLegalEntities = useCallback(async (text: string): Promise<LegalEntity[]> => {
    ...
    try {
      // Utiliser le vrai service OCR pour extraire les entités
      const { processDocumentOCR } = await import('@/services/realOcrService');
      
      // Simuler un fichier temporaire pour l'analyse du texte
      const blob = new Blob([text], { type: 'text/plain' });
      const file = new File([blob], 'analysis.txt', { type: 'text/plain' });
      
      const result = await processDocumentOCR(file);
      const entities: LegalEntity[] = [];
      
      // Convertir les entités détectées vers le format attendu
      if (result.entities) {
        Object.entries(result.entities).forEach(([key, value], index) => {
          if (value && typeof value === 'string') {
            entities.push({
              type: 'legal_reference' as LegalEntity['type'],
              value: String(value),
              confidence: 0.85,
              startIndex: index * 10, // Position approximative
              endIndex: (index * 10) + String(value).length
            });
          }
        });
      }
      
      // Enrichir avec des détections supplémentaires pour le texte français/arabe
      ...
      const amountRegex = /\b\d+(?:\s*\d{3})*(?:,\d{2})?\s*€|\b\d+(?:\s*\d{3})*(?:,\d{2})?\s*(?:DA|دج)|\b\d+(?:\s*\d{3})*(?:\.\d{2})?\s*(?:euros?|dinars?)\b/gi;
      ...
      // Détection des références légales algériennes
      const legalRefRegex = /article\s+\d+(?:-\d+)*(?:\s+du\s+code\s+\w+)?|L\.\s*\d+(?:-\d+)*|R\.\s*\d+(?:-\d+)*|DÉCRET\s+EXÉCUTIF\s+N°\s+\d+-\d+/gi;
