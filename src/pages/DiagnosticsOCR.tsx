...
import { Upload, CheckCircle2, AlertCircle, FileText, Activity } from 'lucide-react';
...
export default function DiagnosticsOCRPage() {
  ...
  useEffect(() => {
    // SEO
    document.title = 'Diagnostic OCR hors ligne — PDF.js & Tesseract';
    const meta = document.createElement('meta');
    meta.name = 'description';
    meta.content = "Vérifiez PDF.js local et Tesseract (FR/AR) pour un OCR 100% hors ligne.";
  ...
  return (
            <PageLayout currentSection="ocr-diagnostics" onRefresh={runDiagnostics}>
      <div className="mt-6 space-y-6">
        <SectionHeader
          icon={FileText}
          title="Diagnostic OCR et Interface Complète"
        ...
        <Tabs defaultValue="upload" className="space-y-4">
          ...
          <TabsContent value="upload">
            <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2"><Activity className="w-5 h-5"/> Indicateurs</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="flex items-center gap-2">
                    {pdfOk ? <CheckCircle2 className="text-green-600"/> : pdfOk === false ? <AlertCircle className="text-red-600"/> : <span className="w-4 h-4 rounded-full bg-gray-300"/>}
                    <span>PDF.js local OK</span>
                  </div>
                  <div className="flex items-center gap-2">
                    {langOk ? <CheckCircle2 className="text-green-600"/> : langOk === false ? <AlertCircle className="text-red-600"/> : <span className="w-4 h-4 rounded-full bg-gray-300"/>}
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2"><FileText className="w-5 h-5"/> Tester un document</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <input 
                    type="file" 
          ...
          <TabsContent value="testing">
            <TestingInterface onRunTest={processFile} />
          </TabsContent>
