...
export function ApprovalQueueModal({ 
  ...
  // Pagination pour tous les éléments
  const {
  ...
  // Pagination pour les textes juridiques
  const {
    currentData: paginatedLegalTexts,
    currentPage: legalCurrentPage,
    totalPages: legalTotalPages,
    itemsPerPage: legalItemsPerPage,
    totalItems: legalTotalItems,
    setCurrentPage: setLegalCurrentPage,
    setItemsPerPage: setLegalItemsPerPage
  } = usePagination({
    data: legalTexts,
    itemsPerPage: 5
  });
          ...
          {filterType === 'all' ? (
            ...
            <TabsContent value="legal-texts" className="space-y-4 mt-6">
              {legalTexts.length === 0 ? (
                ...
                <>
                  {paginatedLegalTexts.map(item => <ItemCard key={item.id} item={item} />)}
                  ...
                  <div className="mt-6">
                    <Pagination
                      ...
                      totalItems={legalTotalItems}
                      itemsPerPage={legalItemsPerPage}
                      onPageChange={setLegalCurrentPage}
                      onItemsPerPageChange={setLegalItemsPerPage}
                    />
                  </div>
                </>
              )}
            ...
            <TabsContent value="procedures" className="space-y-4 mt-6">
              {procedures.length === 0 ? (
                ...
                <>
                  {paginatedProcedures.map(item => <ItemCard key={item.id} item={item} />)}
            ...
            // Affichage direct des éléments filtrés avec pagination
            <div className="space-y-4 mt-6">
              {filteredItems.length === 0 ? (
                ...
                <>
                  ...
                  {/* Pagination pour les éléments filtrés */}
                  <div className="mt-6">
                    <Pagination
                      currentPage={allCurrentPage}
                      totalPages={allTotalPages}
