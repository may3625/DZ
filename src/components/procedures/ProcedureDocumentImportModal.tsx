...
interface ProcedureDocumentImportModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function ProcedureDocumentImportModal({ isOpen, onClose, onImportSuccess }: ProcedureDocumentImportModalProps) {
  ...
  const documentTypes = [
    { value: 'procedure_guide', label: 'Guide de procédure' },
    { value: 'formulaire', label: 'Formulaire' },
    { value: 'modele_document', label: 'Modèle de document' },
    { value: 'attestation', label: 'Attestation' },
    { value: 'fiche_procedure', label: 'Fiche procédure' },
  ...
  const handleImport = async () => {
    if (files.length === 0) {
      toast({ title: 'Aucun fichier sélectionné', description: 'Veuillez sélectionner au moins un fichier à importer.', variant: 'destructive' });
      return;
    }
    if (!documentType) {
      toast({ title: 'Type de document requis', description: 'Veuillez sélectionner le type de document.', variant: 'destructive' });
      return;
    }
    setIsImporting(true);
    try {
      ...
      if (successfulFiles.length > 0) {
        toast({ title: 'Importation réussie', description: `${successfulFiles.length} fichier(s) importé(s) avec succès.` });
        if (onImportSuccess) onImportSuccess(successfulFiles);
      }
      if (failedFiles.length > 0) {
        toast({ title: "Erreurs d'importation", description: `${failedFiles.length} fichier(s) n'ont pas pu être importés.`, variant: 'destructive' });
      }
      if (failedFiles.length === 0) {
        setTimeout(() => { handleClose(); }, 1500);
      }
    } catch (error) {
      toast({ title: "Erreur d'importation", description: "Une erreur est survenue lors de l'importation des fichiers.", variant: 'destructive' });
    } finally {
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-3 text-xl">
            ...
            Importer des documents de procédures
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button onClick={handleImport} disabled={isImporting || files.length === 0}>
