...
function LegalTextFormEnhanced({ 
  ...
  const { getLegalTextTypes, getFormById } = useEnrichedFormLibrary();
  ...
  // Obtenir les types de textes depuis la bibliothèque enrichie
  const legalTextTypes = getLegalTextTypes();
  // Éviter les doublons en utilisant un Map avec l'ID comme clé
  const uniqueLegalTextTypes = Array.from(
    new Map(legalTextTypes.map(type => [type.id, type])).values()
  );
  const availableTypes = uniqueLegalTextTypes.map(type => type.name);
  const availableCategories = Array.from(new Set(uniqueLegalTextTypes.map(type => type.category)));
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
      ...
      console.log('Types de textes juridiques enrichis:', legalTextTypes.length);
      console.log('Types uniques après déduplication:', uniqueLegalTextTypes.length);
  ...
  }, [customForms.length, legalTextTypes.length, uniqueLegalTextTypes.length, availableTypes.length, availableCategories.length, availableTypes, availableCategories]);
  ...
  const handleOCRFormDataExtracted = useCallback((data: { documentType: 'legal', formData: Record<string, any> }) => {
    ...
    if (!formToUse) {
      ...
      if (ocrData.type || ocrData.TYPE) {
        ...
        formToUse = uniqueLegalTextTypes.find(type => 
          type.template.type === detectedType || 
          type.template.type.toLowerCase().includes(String(detectedType).toLowerCase()) ||
          String(detectedType).toLowerCase().includes(type.template.type.toLowerCase())
        )?.template;
      }
      
      // Si toujours pas trouvé, utiliser le premier formulaire juridique disponible
      if (!formToUse && uniqueLegalTextTypes.length > 0) {
        formToUse = uniqueLegalTextTypes[0].template;
      }
  ...
  }, [legalTextTypes, selectedForm, setInputMethod, toast, mapAlgerianOCRDataToForm, mapOCRDataToForm, selectedTextType, validateAlgerianDocument]);
  ...
  useEffect(() => {
    if (selectedTextType) {
      const type = uniqueLegalTextTypes.find(t => t.name === selectedTextType);
      if (type && type.template) {
        setSelectedForm(type.template);
        // Initialiser les données du formulaire avec des valeurs vides
        const initialData: { [key: string]: string } = {};
        (type.template.fields as any[]).forEach((field: Record<string, unknown>) => {
  ...
  }, [selectedTextType, uniqueLegalTextTypes]);
