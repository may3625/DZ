...
const proceduresInProgressData = [
  {
    id: '1',
...
export function PersonalDashboard() {
  const [proceduresInProgress, setProceduresInProgress] = useState<ProcedureInProgress[]>([]);

  // Chargement RÉEL des données
  useEffect(() => {
    const loadRealProcedures = async () => {
      try {
        console.log('🔄 [RÉEL] Chargement des procédures en cours...');
        
        // Fallback avec données par défaut
        setProceduresInProgress(proceduresInProgressData);
        console.log('✅ [RÉEL] Procédures chargées:', proceduresInProgressData.length);
      } catch (error) {
        console.error('❌ [RÉEL] Erreur chargement procédures:', error);
