...
export function ComplementaryResourcesSection({ language = "fr" }: ComplementaryResourcesSectionProps) {
  ...
  // Unifié: possibilité d'ouvrir via provider (sans changer l'UI actuelle)
  const { openModal, closeModal } = useModal();

  const openAddSourceUnified = () => {
    const id = `add-source-${Date.now()}`;
    openModal({
      id,
      type: 'display',
      title: 'Ajouter une Source',
      content: (
        <AddSourceModal isOpen={true} onClose={() => closeModal(id)} />
      ),
      size: 'lg'
    });
  };
  ...
  return (
    <div className="space-y-6">
      <Tabs defaultValue="sources" className="w-full">
        ...
        <TabsContent value="sources" className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <Button className="ml-4" onClick={openAddSourceUnified}>
      ...
      {/* Modales */}
      {/* Ouverture via ModalProvider désormais. Fallback local retiré. */}
