...
export function MobileAppSection({ language = "fr" }: MobileAppSectionProps) {
  ...
  const triggerLocalAlert = () => {
    toast({
      title: 'Alerte locale',
      description: "Ceci est une notification locale hors‑ligne.",
    });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Outils mobiles (voix, QR, notifications locales) */}
      <Card>
        <CardHeader>
          <CardTitle>Outils mobiles</CardTitle>
          <CardDescription>Recherche vocale, scanner QR local et alertes in‑app</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label className="font-medium">Recherche vocale</Label>
            <div className="max-w-xl">
              {/* Réutilise le composant commun déjà présent */}
              {/* @ts-ignore - component provides its own internal mic handling */}
              <VoiceSearchInput
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Label className="font-medium">Scanner QR (local)</Label>
              <p className="text-sm text-muted-foreground">Ouvrez un lien local via un QR</p>
            </div>
            <Button variant="outline" onClick={() => setShowQR(true)}>Scanner QR</Button>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Label className="font-medium">Notification locale</Label>
              <p className="text-sm text-muted-foreground">Alerte visible hors‑ligne</p>
            </div>
            <Button variant="outline" onClick={triggerLocalAlert}>Tester</Button>
          </div>
          ...
          {showQR && (
            <QRScanner
              onDetected={(text) => {
                setShowQR(false);
                try {
                  // Tente d'ouvrir si c'est un lien; sinon, affiche en toast
                  const url = new URL(text);
                  window.open(url.toString(), '_blank');
                  toast({ title: 'QR détecté', description: url.toString() });
                } catch {
                  toast({ title: 'QR détecté', description: text });
                }
              }}
