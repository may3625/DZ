...
export function LegalTextsSearchHistoryTab() {
  const [searchVersions, setSearchVersions] = useState<LegalTextSearchVersion[]>([]);

  // Chargement RÉEL des données
  useEffect(() => {
    const loadRealSearchVersions = async () => {
      try {
        console.log('🔄 [RÉEL] Chargement des versions de recherche...');
        // Utiliser données par défaut
        setSearchVersions(searchVersionsData);
        console.log('✅ [RÉEL] Versions de recherche chargées:', searchVersionsData.length);
      } catch (error) {
        console.error('❌ [RÉEL] Erreur chargement versions de recherche:', error);
        setSearchVersions([]);
      }
    };
    
    loadRealSearchVersions();
  }, []);
  ...
  const filteredVersions = searchVersions.filter(version => {
