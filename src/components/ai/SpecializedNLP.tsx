...
export function SpecializedNLP() {
  ...
  const extractAdvancedEntities = async (text: string): Promise<LegalEntity[]> => {
    const entities: LegalEntity[] = [];
    
    // Patterns améliorés pour l'extraction d'entités juridiques RÉELLES
    const patterns = [
      {
  ...
  const generateAutoSummary = async (text: string): Promise<AutoSummary> => {
  ...
  const classifyDocument = async (text: string): Promise<DocumentClassification> => {
  ...
  const analyzeSentiment = async (text: string): Promise<EnhancedSentimentAnalysis> => {
  ...
  const handleNLPAnalysis = async () => {
    ...
    try {
      const [entities, summary, classification, sentiment] = await Promise.all([
        extractAdvancedEntities(inputText),
        generateAutoSummary(inputText),
        classifyDocument(inputText),
