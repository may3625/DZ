...
import { ExtractionJobCard } from '@/components/extraction/ExtractionJobCard';
...
export function AutoExtractionModal({ isOpen, onClose, context }: AutoExtractionModalProps) {
  ...
  const { scanDocument } = useOCRScanner();
  ...
  const handleStartExtraction = async () => {
    ...
    try {
      ...
      for (const source of sources) {
        ...
        // Traitement réel basé sur le type de source
        let realResults: Record<string, unknown>[] = [];
        
        if (activeTab === 'files' && batchFiles) {
          // Traitement OCR pour les fichiers
          
          for (const file of Array.from(batchFiles)) {
            try {
              const ocrResult = await scanDocument(file);
              if (ocrResult) {
                realResults.push({
                  fileName: file.name,
              ...
              console.error('Erreur OCR pour', file.name, error);
            }
          }
        } else {
          // Pour les URLs et APIs, essayer d'extraire du contenu réel quand possible
          console.warn('⚠️ [AutoExtraction] Extraction URL/API non implémentée - utilisation de données de démonstration');
          realResults = generateDemoResults(context);
        }
        
        setExtractionJobs(prev => prev.map(job => 
          job.id === jobId 
            ? { ...job, results: realResults, extractedCount: realResults.length }
            : job
        ));
      }
  ...
  const generateDemoResults = (context: string) => {
