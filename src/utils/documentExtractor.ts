...
export class DocumentExtractor {
  ...
  /**
   * Extraction depuis PDF (utilise le service réel d'extraction PDF)
   */
  private static async extractFromPDF(file: File): Promise<ExtractedDocument> {
    try {
      logger.info('OCR', '🔄 Utilisation du service réel d\'extraction PDF:', file.name);
      
      // Utiliser le service réel d'extraction PDF (100% local)
      const { extractPDFDataLocally } = await import('@/services/pdfExtractionService');
      const result = await extractPDFDataLocally(file);

      return {
        text: result.text,
        format: 'PDF (Extraction réelle)',
        metadata: { 
      ...
      logger.error('OCR', 'Erreur extraction PDF réelle:', error);
      throw new Error(`Erreur lors de l'extraction réelle du PDF: ${error}`);
    }
  }
      ...
      return new Promise((resolve, reject) => {
        img.onload = async () => {
          try {
            ...
            // Utiliser Tesseract.js pour l'OCR (français uniquement pour éviter les erreurs 404)
